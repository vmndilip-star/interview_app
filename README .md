...
**What's new**
- **PDF resumes** — upload a PDF or `.txt` (or still paste text). The job
...
- **Token optimization** — the raw resume + JD are compressed **once** at
  session start into a compact profile that the interviewer reuses every turn,
  instead of re-sending the full text. History is trimmed to the last few turns,
  with a running "already asked" list preventing repeats. Cuts per-turn input
  tokens ~5–6× over a full interview.
...
> rule: run `pip` and `streamlit` from the **same** terminal/environment. On
> conda, prefer `python -m pip install ...` so packages land in the active env.
...
2. **Interview screen** — for each question:
   ...
   - Click **Submit answer** (blank answers are blocked). A new question appears
     with a fresh, empty box and mic.
...
4. Answer each question by **typing or sending a voice message**. `/end`
   finishes early; `/cancel` aborts. You get the same scored summary at the end.
...
is lost even if a session ends early. Both front ends write to the **same**
database. Two tables:
...
Or use the **Download all Q&A (JSON)** button on the Streamlit results screen.
...
| Bot ignores a sent PDF | Document handler not registered / wrong order | `filters.Document.ALL` must be listed **before** the text handler in the `ASK_RESUME` state |
...
| Transcription is slightly wrong | Whisper mishears proper nouns | Expected — edit the text box before submitting; the evaluator also ignores minor term slips |
...
- **Models** — in `llm.py`: `INTERVIEWER_MODEL`, `EVALUATOR_MODEL`, and
  `PROFILE_MODEL` (all default `gpt-4o-mini`, cheap and fine for a prototype).
  Bump the evaluator for stricter scoring.
- **Profile compression** — the `compress_profile()` prompt in `llm.py` controls
  what the interviewer sees. Loosen the ~250-word cap or add a section if
...
- **Questions per phase** — the `turns_in_phase >= 4` cap in `app.py` / `bot.py`
  controls how long each phase runs before moving on.
